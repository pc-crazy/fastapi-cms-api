...
def create_user_and_blog(client, email="liker@example.com", password="likepass", is_public=True):
    ...
    login = client.post("/v1/accounts/login", data={
        "username": email,
        "password": password,
        "grant_type": "password"
    })
    token = login.json()["access_token"]
    post = client.post("/v1/blog", json={
        "title": "Like Test",
...
def test_like_post(client):
    ...
    like_res = client.post(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token}"})
...
def test_like_post_twice(client):
    ...
    client.post(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token}"})
    like_again = client.post(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token}"})
...
def test_unlike_post(client):
    ...
    client.post(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token}"})
    unlike_res = client.delete(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token}"})
...
def test_like_invalid_post(client):
    ...
    res = client.post("/v1/like/9999", headers={"Authorization": f"Bearer {token}"})
...
def test_unlike_invalid_post(client):
    ...
    res = client.delete("/v1/like/9999", headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 404
    assert res.json()["detail"] == "Post not found"
...
def test_like_private_post_not_owner(client):
    ...
    client.post("/v1/accounts", json={"name": "Other", "email": "other@example.com", "password": "pass"})
    login = client.post("/v1/accounts/login", data={"username": "other@example.com", "password": "pass", "grant_type": "password"})
    token2 = login.json()["access_token"]
    res = client.post(f"/v1/like/{blog_id}", headers={"Authorization": f"Bearer {token2}"})
