...
def create_user_and_blog(
    ...
    client.post(
        "/v1/accounts",
        json={"name": "Liker", "email": email, "password": password},
    )
    login = client.post(
        "/v1/accounts/login",
        data={
...
def test_like_post_twice(client):
    token, blog_id = create_user_and_blog(
        client, email="liketwice@example.com"
    )
    client.post(
        f"/v1/like/{blog_id}",
        headers={"Authorization": f"Bearer {token}"},
...
def test_like_invalid_post(client):
    ...
    res = client.post(
        "/v1/like/9999", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_like_private_post_not_owner(client):
    ...
    token1, blog_id = create_user_and_blog(
        client, email="owner@example.com", is_public=False
    )
    ...
    assert (
        res.json()["detail"] == "You cannot like a private post you don't own"
    )
